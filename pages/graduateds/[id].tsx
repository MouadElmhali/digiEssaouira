...
import { EmailShareButton, FacebookShareButton } from "next-share";
...
export async function getServerSideProps({ params: { id } }: IContext) {
...
export default function Association({
  ...
  const { post, name, bio, pictureUrl, facebook, gmail, instagram } =
  ...
  const [shareLink, setShareLink] = useState({
    title: "DigiEssaouira | خريجي DIGIESSAOUIRA" + " : " + name,
    url: router.pathname,
  });
  ...
  return (
    <>
      <Head>
        <title>DigiEssaouira | خريجي DIGIESSAOUIRA</title>
      </Head>

      <main className="mt-20 h-[75vh] min-h-[950px] md:min-h-[150px]  lg:min-h-[650px] container mx-auto px-10 ">
        <Section className="py-20" childrenClassName="w-full md:px-24">
          <div className="flex flex-col items-center gap-y-5  gap-x-8 sm:grid sm:grid-cols-[auto,_1fr] sm:grid-rows-[repeat(4,_auto)] shadow-w">
            ...
            <div className="flex flex-col gap-3">
              ...
              ></div>
              <div className="flex gap-3 self-center md:self-end relative sm:top-[30px] lg:top-[130px]">
                <FacebookShareButton
                ...
                <button
                  className="flex items-center justify-center bg-primary  text-white font-bold p-2 rounded-full "
                  onClick={handleShare}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
