    ...
    return (
        <div className="container mx-auto px-5 md:px-28 flex flex-raw gap-8 ">
            ...
            <div className="hidden lg:flex lg:flex-col h-92  w-1/3 mt-48 mb-48">
                ...
                <p className="mt-5 text-justify 2xl:min-w-[30rem]">
                    {aboutus}
                </p>
                ...
                
                {
                    ...
                    <div>
                        ...
                        {articles.slice(articles.length - 3, articles.length).map(({title, pictureUrl, id }) => (
                            <button key={id} onClick={() => router.push("/youth/" + id)} className='flex flex-row items-center 2xl:min-w-[30rem]  gap-5 transition duration-200 ease-in-out hover:bg-gray-100 overflow-hidden rounded-lg  py-3 px-2 '>
                                <img src={"/images/articles/" + pictureUrl} className="object-cover max-h-16 min-w-[8rem] rounded" />
